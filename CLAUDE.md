...
## แผนถัดไป (ยังไม่ทำ — มีสเปคแล้ว)
- **รับชำระนอกแพลตฟอร์ม 2 แบบ** (เจ้าของสั่ง 2026-07-23) — สเปคเต็มที่ **`for-design/direct-transfer-cod-brief.md`**
  1. **แท็บใหม่ "ลูกค้าโอนตรงเข้าบริษัท"** (ข้างแท็บ 🛒 ถอน Marketplace) — คัดออเดอร์ BigSeller ที่ไม่ใช่แพลตฟอร์ม + ชำระแบบโอนตรง → จับกับเงินเข้าในสเตทเมนต์ (วัน+ยอด+**ชื่อผู้โอน**) → การเงินติ๊ก "ตรวจ/ยืนยัน" → ไหลไปหน้า **2. รับชำระเงิน** ส่งออก RE (โอนตรง = **ไม่มีเช็ค/BQ · ไม่มีค่าธรรมเนียม**)
     - ★ **ห้าม auto-confirm เมื่อกำกวม** (วันเดียวกันยอดซ้ำได้ เช่น คีย์ 5 ใบยอด 390 แต่เงินเข้า 4 ครั้ง) — จับผิด = ลูกหนี้คงค้างผิด ต้องให้คนเลือกคู่เอง
     - ต้องเก็บเพิ่มตอน import BigSeller: **ประเภทการชำระเงิน · ชื่อผู้ซื้อ/ผู้โอน · วันที่โอนที่ฝ่ายขายกรอก** (`ORD_CH.bigseller` ยังไม่ map · `order_ledger.customer` ว่างสำหรับออเดอร์จาก BigSeller) → ต้อง migration + ขอไฟล์ BigSeller ตัวอย่างดูชื่อคอลัมน์จริง
  2. **COD ผ่านขนส่ง** (ขาย FACE/LINE เก็บเงินปลายทาง) — ขนส่งโอนคืนเป็นก้อนหลังหักค่าบริการ → แมพว่าเงินก้อนนั้นมีออเดอร์/RE ไหนบ้าง → ผ่านเช็ค (BQ) ยอดรวม หักค่าใช้จ่ายที่หน้าผ่านเช็ค · โครงเดียวกับ `bmpGroupWithdrawals` (reuse ได้) · **รอไฟล์รายงานขนส่งตัวอย่างจากเจ้าของ**
...
- **`catbotAcctStats(a)`** = Σรับ/Σจ่าย + `closing = opening+in−out` เทียบ **ยอดคงเหลือแถวสุดท้ายในไฟล์** (`lastBal`) → `diff`/`ok` (tolerance 0.02)
- **ชิป "🏦 ยอดยกมา"** (`catbotToggleAccts` · `cb.acctOpen` · **เปิดเองอัตโนมัติเมื่อ `acctBad>0`** = ไม่ลงตัว หรือ opening=0) → การ์ดรายบัญชี: ยอดยกมา (input แก้ได้) · รับ · จ่าย · คงเหลือปลายงวด · badge ✓ตรง / ⚠ต่าง N / ไฟล์ไม่มียอดคงเหลือ
- **`catbotSetOpening(ai,val)`** (strip `, ฿` · เซฟ session อัตโนมัติ) · **`catbotFixOpening(ai)`** = คำนวณย้อนกลับ `opening = lastBal − in + out` (ไฟล์ไม่มี balance → alert ให้กรอกเอง)
- แก้แล้วต้อง **ส่งออก/ยืนยันใหม่** เพื่อดันค่าที่ถูกเข้า Executive Dashboard
- **verified:** unit test 10 เคส (จับ diff −300,000 · คำนวณย้อนกลับได้ 300,000 · ไฟล์ไม่มี balance ไม่ฟ้องผิด · panel เปิด/ปิดเองถูก) · syntax OK

### 2026-08-05 (4) — ★★ งบกระแสเงินสด (Exec): อัปหลายไฟล์ + ไฟล์แยกบัญชีเดือนเดียวกันทับกันหาย (b3007z)
- **เจ้าของ:** โหลดไฟล์แยกตามบัญชี 3 ไฟล์ (0405 / 4889 / 1640) → "บังคับอัปทีละไฟล์ · อัปไฟล์ใหม่ไฟล์เก่าหาย เหมือนเซฟทับ" · Transaction Explorer SCB 417-077164-0 = **0 รายการ**
- **★ ต้นเหตุ (`edMergeData`):** merge ด้วย `monthMap[m.key] = m` = **ทับทั้งเดือน** · ไฟล์ทั้ง 3 เป็นเดือนเดียวกัน (2026.07) คนละบัญชี → อัปไฟล์ที่ 2 = บัญชีของไฟล์ที่ 1 หายเกลี้ยง (คอมเมนต์เดิมเขียนไว้ว่า "เดือนซ้ำของใหม่ทับ" — ถูกสำหรับเคสอัปคนละปี แต่พังกับไฟล์แยกบัญชี)
- **แก้ `edMergeMonth(oldM,newM)`** — เดือน key ซ้ำ = **รวมรายบัญชี (`accountKey`)** · บัญชีเดียวกันเดือนเดียวกัน = ของใหม่ทับ (อัปซ้ำเพื่อแก้ไขได้ ไม่บวกซ้ำ) · `edRebuildFromMonths` rebuild transactions/accounts ให้เอง (ไม่ซ้ำ)
- **อัปหลายไฟล์ทีเดียว:** `edHandleFile` คืน **Promise** + `opts.silent` · `edHandleFiles(files)` วนทีละไฟล์ (silent) แล้ว `edSave/edSaveCloud/render` **ครั้งเดียวตอนจบ** · ปุ่ม "อัปโหลดเพิ่ม" + dropzone + `#edInput` เพิ่ม `multiple`
- **`edMergeSummary(data)`** — ข้อความสรุปบอก ปี/เดือน/**จำนวนบัญชี** ที่มีตอนนี้ ("ของเดิมยังอยู่ครบ") แทนข้อความเดิมที่บอกแค่ปี
- **★ ไม่แตะ logic การรวมเลข/แสดงผลของ Exec Dashboard** (แก้เฉพาะ import path — ตามแนวเดียวกับ 2026-07-03)
- **verified:** unit test 8 เคส (อัป 3 ไฟล์ → 3 บัญชีอยู่ครบ · transactions ไม่ซ้ำ · อัปบัญชีเดิมซ้ำ = ทับไม่บวกเบิ้ล · เดือนใหม่ไม่ทับเดือนเก่า)
- **หมายเหตุ:** ไฟล์ที่เจ้าของมี (`0405undefined_รายละเอียดเงินรับเงินจ่าย_2026.07.xlsx`) **ไม่ได้มาจาก `catbotExportExcel`** ของ repo นี้ (ตัวนั้นตั้งชื่อ `{CODE}_รายละเอียดเงินรับเงินจ่าย_{month}.xlsx` และส่งออกไฟล์เดียวรวมทุกบัญชี · `git log -S` ยืนยันบรรทัดนี้ไม่เคยเปลี่ยน) — ยังไม่ทราบที่มา
...
- **ค่าธรรมเนียม "ครบ" อยู่แล้วทุกที่:** `sales_income_rows` รวมทุกรอบบิลของ order_id เดียวกัน (`incIngestRows` merge by order_id) · `order_ledger.receipt_fee` มาจาก 191 (เลขจริง) · ยอดรวมใบถอนตรง STM (ค่าส่งรอบหลังอยู่ในกลุ่ม adj)
- **ปัญหาเดียว = มูลค่าเช็คในไฟล์ส่งออก BQ:** ใช้ `shopee_net` ของรอบบิลนั้น (532.78) แต่ **เช็คจริงที่บัญชีคีย์ = 500.68** (เจ้าของยืนยัน)
- **แก้ (`bmpGroupWithdrawals`):** รวม txn ที่ `order_id` เดียวกัน **ภายในรอบถอนเดียวกัน** → เช็คใบเดียว · เก็บแถวยอดมากสุดเป็นตัวหลัก แล้วบวกยอดที่เหลือเข้าไป · ลบแถวที่เหลือ
  - **รวมหลังแบ่งงวดแล้ว** → ออเดอร์ที่ตกคนละรอบถอนไม่ถูกรวม (23/26 รวมได้ · 2 ใบคนละรอบถอน = เงินเข้าคนละงวดจริง)
  - **★ เฉพาะ Lazada** (`routing.channel === "lazada"`) — Shopee/TikTok เงินเข้าออเดอร์ครั้งเดียว ไม่แตะ
  - ยอดรวมใบถอนไม่เปลี่ยน (แค่ยุบแถว) → ยังตรง STM
- **ค่าโฆษณา Lazada** (`Payment / Sponsored Solutions Top-up`) ไม่มี order_id → ไม่ถูกรวม · ตกเป็น adjustment ตามเดิม ✓
- **APP_BUILD 3007v** · unit test 3 เคส (รวมได้ 500.68 / Shopee ไม่แตะ / ค่าโฆษณาไม่ถูกรวม)

### 2026-08-03 — ★★★ "ยอดยกมา / ออเดอร์ 0" ทั้งหน้าถอน Marketplace = โหลดออเดอร์ไม่แบ่งหน้า (cap 1000)
- **อาการ (เจ้าของ):** ใบถอนขึ้น "⚠ ยอดยกมา/ออเดอร์ก่อนงวด (ไม่มีในไฟล์)" + **ออเดอร์ 0** เกือบทุกใบ · อัปไฟล์ซ้ำหลายรอบไม่หาย · "ตรงครบ" ตกจาก 20 → 12
- **★ ต้นเหตุจริง (`bmpLoadFromDb`):** `sb.from("brec_mp_orders").select("*").in("withdrawal_id", wdIds)` **ไม่แบ่งหน้า** → PostgREST ตัดที่ **1000 แถว** · Benya มีออเดอร์รวม **3,798** (betra 3054 + QI 744 + tiktok) → ใบถอนหลัง ๆ ไม่มีออเดอร์ติดมา → `sumCheque=0` → `walletExpense` ติดลบเต็มจำนวน → โชว์ "ยอดยกมา" **ทั้งที่ข้อมูลใน DB ครบ**
  - แก้: แบ่งหน้า `.range()` ทั้ง `brec_mp_orders` + `brec_bank_rows` (เงินเข้าเกิน 1000 แถว = จับ STM ไม่เจอแบบเงียบ ๆ) · **ไม่ต้องอัปไฟล์ใหม่ แค่ refresh** · บั๊กชนิดเดียวกับ H3/H4 (2026-07-28) ที่จุดนี้ตกหล่น
- **แก้พ่วง (b3007q · ยังคงไว้ · ถูกต้องขึ้นจริง):** การแบ่งงวดเปลี่ยนเป็น **FIFO ตามมูลค่า** (ดันออเดอร์ใหม่สุดกลับทีละใบ ตราบใดที่ยังคลุมยอดถอน) แทน subset-sum + สาขา `balance_after` · + **seed เงินค้างกระเป๋าก่อนแถวแรกของไฟล์** = `balance_after(แถวแรก) − amount(แถวแรก)` (BETRA 34,642 · QI 29,083.71 จากออเดอร์ เม.ย.) · TikTok/Lazada `balance_after=0` → seed=0 (พฤติกรรมเดิม)
  - verified ไฟล์จริง: ใบที่ขึ้น "ยอดยกมา" 13 → 1 · ก.ค. BETRA ตรงเป๊ะทุกใบ · unit test 4 เคส
- **★★ บทเรียน (เสียเวลาหลายชั่วโมง):** ผมไล่หาที่ "ตอนคำนวณ/จับกลุ่ม" ตลอด ทั้งที่อาการคือ **"ข้อมูลถูกตอนอัป แต่ผิดตอนแสดง"** → ต้องสงสัย **ฝั่งอ่าน (read path)** ก่อน · และควรเอา **ไฟล์ต้นทางมาคำนวณ ground truth** ตั้งแต่ชั่วโมงแรก (ไฟล์กระเป๋าเงิน Shopee มีคอลัมน์ยอดคงเหลือทุกแถว = ตรวจสอบได้ 100%) แทนการอ่านโค้ดเดาทีละสมมติฐาน
- **APP_BUILD 3007r** · เจ้าของยืนยัน: ตรงครบเกือบทุกใบ · ที่เหลือไม่ตรงเพราะ "ยังไม่ออกใบเสร็จ RE" (ความจริง ไม่ใช่อาการหลอก)
- **★ ตรวจสอบโมเดลกับไฟล์จริง + การแมพมือของเจ้าของ (Shopee QI/BETRA พ.ค.–ก.ค.):**
  - **FIFO ตามมูลค่า = วิธีเดียวกับที่เจ้าของแมพมือ** — เทียบจำนวนใบต่อรอบ: 06/07 47=47 · 14/07 55=55 · 16/07 32=32 · 20/07 47=47 · 27/07 53=53 (**ตรงเป๊ะ**) · ต่างเฉพาะรอบที่มี "ออเดอร์คร่อมรอบ"
  - **BETRA ระบบจับได้เอง 15/16 ใบ ตรงทุกบาท** · QI 16/17 · ที่เหลือคือ**ใบถอนแรกสุดของไฟล์** (เงินมาจากออเดอร์เดือนก่อนที่ไม่อยู่ในไฟล์: BETRA 34,642 · QI 29,083.71) → ไม่ใช่บั๊ก · แก้โดยอัปกระเป๋าเงินเดือนก่อนเพิ่ม
  - **★ เศษ 0.71 ของ QI = เศษถาวรติดมาก่อน พ.ค. ไม่ได้เกิดใหม่** · ข้อมูลจริง **ไม่มีรายการเป็นเศษสตางค์เลย** (QI 813 แถว · BETRA 3,213 แถว = จำนวนเต็มล้วน) · "ถอนจนเหลือ 0" ในทางปฏิบัติ = เหลือ 0.71
  - **★★ โค้ดปัจจุบันไม่มี threshold ผูกกับเศษใด ๆ** (ถอด `balance_after > 0.5` ทิ้งตอน 3007q) → เศษเปลี่ยนเป็นเท่าไรก็ไม่เพี้ยน (ทดสอบ 0.71→1.21 · 13.72 · 0 = ผ่านหมด) · **ห้ามใส่ threshold เทียบเศษกลับเข้ามา**
  - **ออเดอร์คร่อมรอบ (แอดมินเก่าถอนไม่หมด):** เจ้าของแมพมือแบ่ง 1 ออเดอร์เป็น 2 รอบได้ (478 = 397 รอบ 13/7 + 81 รอบ 14/7) แต่**ระบบแบ่งเช็คครึ่งใบไม่ได้** → ให้ทั้งใบกับรอบเดียว + ยกเศษไปรอบหน้า (ยอดรวมยังตรง)
  - **★ b3007s — เลือก "ใบที่ยอดพอดี" ยกไปงวดหน้า:** เดิมดันใบ*ใหม่สุด*กลับ → เศษแปลก (QI 06/07 = 26.71) · ความจริง Shopee ทิ้ง**ออเดอร์ใบที่ยอดตรงกับเงินเหลือ** ไว้ในกระเป๋า (เหลือ 527.71 = ออเดอร์ 527 ใบเดียว · เจ้าของทำสีเขียวยืนยันในไฟล์) → เพิ่ม: ก่อน FIFO หาออเดอร์ใบเดียวที่ยอด ≤ ที่ต้องยก และเหลือเศษ < 1 บาท → ยกใบนั้น · ไม่เจอ = FIFO เดิม
    - **blast radius แคบมาก** (ทดสอบ QI+BETRA 33 รอบ): เปลี่ยน **1 รอบ** (QI 06/07 26.71→0.71) · รอบอื่นไม่ขยับ · เจ้าของเลือกทางนี้เอง (ตัวเลือก ข) แทนการเปลี่ยนวิธีเลือกทั้งหมดที่ทำ QI 13/07 แย่ลง
  - **★★ b3007t — ยกออเดอร์แบบ "พอดีที่สุด" (best-fit) แทน "ใบใหม่สุด":** เจ้าของจับได้ว่า BETRA 16/07 **ยอดคงเหลือในกระเป๋า = 0 แต่ระบบยกไป 397** · ต้นเหตุ: loop เดิมดูแค่ใบใหม่สุด ถ้าใบนั้นใหญ่เกินยอดที่ต้องยกก็ `break` ทันที **ไม่ไปหาใบเล็กกว่าในกอง** → เศษค้างเกินจริงแล้วลามงวดถัดไป
    - แก้: เลือก **ใบที่ใหญ่สุดที่ยังยกได้** ทำซ้ำจนไม่มีใบไหนยกได้ + sort คิวตามเวลา (แทน newest-only และ single-exact-fit ของ b3007s ซึ่งกลายเป็นกรณีย่อย)
    - verified ไฟล์จริง: BETRA เศษรวม **1,048 → 22** · ทุกใบ ก.ค. = 0 · QI 06/07 26.71 → 0.71 · unit test 4 เคส
    - **★ เกณฑ์ตรวจงานที่เจ้าของชี้ (ใช้ต่อไป): "ยอดยกไป" ต้องเท่ากับ `balance_after` ของแถวถอนเสมอ** — ถ้าไม่เท่า = การแบ่งงวดเพี้ยน
- **นโยบายต่อไป (เจ้าของยืนยัน):** การเงินถอน**จนเหลือ 0** ทุกครั้ง → 1 รอบ = ทุกออเดอร์ตั้งแต่ถอนครั้งก่อน พอดีเป๊ะ → ไม่มีออเดอร์คร่อมรอบ → ยกมา/ยกไป = 0 (ระบบจับเองได้ 100% ไม่ต้องแมพมือ)

### 2026-08-02 — ★★ RE ผูกผิดใบ 340 ใบ (บล็อกส่งออก RE ทั้งระบบ) + 191 สะสม + บทเรียน blast radius
- **อาการ (เจ้าของ):** ส่งออก RE ไม่ได้สักใบ ("พร้อมออก RE = 0") ทั้งที่หลายใบยังไม่ได้คีย์รับชำระจริง · Bank Recon บอก "ยังไม่ออก RE" แต่หน้ารับชำระบอก "รับชำระแล้ว" — **2 หน้าขัดกัน**
- **ต้นเหตุ (พิสูจน์กับไฟล์ 191 จริง):** ออเดอร์ **340 ใบติดเลข RE ของใบอื่น** — มาจาก parser 191 เวอร์ชันเก่าจับคู่ RE↔IV เลื่อนแถว (แก้ไปแล้ว 2026-07-21) แต่ **ข้อมูลผิดค้างใน `order_ledger`** · ตัวกรองส่งออก = `!re_no` → มี re_no (แม้ผิด) = ถูกตัดออกหมด
  - verified 3/3: RE2607000529→IV2607000701 · RE2607000531→IV2607000695 · RE2607000516→IV2607000576 (ตรงกับที่เครื่องมือตรวจบอก) · IV ของออเดอร์ที่ถูก flag ไม่มีใน 191 เลย = ยังไม่คีี่ย์จริง
- **เครื่องมือใหม่ "ตรวจ RE ผูกผิดใบ"** (`incLoadReEvidence`/`incClassifyReTags`/`incOpenReAudit`/`incClearWrongRe`/`incReAuditPanelHtml` · ปุ่มในแท็บ "จับคู่ IV↔Income"):
  - หลักฐาน = **191 สะสม (`brec_mp_express_cache`) + 1.9.1 ที่อัปในแท็บตรวจการคีย์** → map `IV↔RE`
  - จำแนก 3 กลุ่ม: **ยืนยันถูกต้อง** (RE ตรงหลักฐาน) · **ผูกผิดใบ** (RE นี้เป็นของ IV อื่น = พิสูจน์ได้) · **★ ไม่มีหลักฐาน = ไม่ตัดสิน ไม่แตะ** (fail-safe)
  - ติ๊กเลือก → ล้าง `re_no/re_date/cheque_no/receipt_*/received_at` (เขียน DB เฉพาะที่ผู้ใช้ติ๊กเอง) → กลับไป "พร้อมออก RE"
  - ผลจริง Benya: ยืนยันถูก **2,677** · ผูกผิด **340** · ไม่มีหลักฐาน 4 (หลักฐาน 4,370 RE)
- **★ 191 สะสมไม่ทับ (commit 407794f):** เดิมอัป Express ทับ cache ชุดเดียว → อัปเฉพาะวัน = วันอื่นหาย · เปลี่ยนเป็น merge (key: doc_no / IV / เลขเช็ค · ใบใหม่ทับ = newer wins) → **อัปเฉพาะวัน/ช่วงได้ ไม่ต้องอัปทั้งเดือนทุกครั้ง** · + ปุ่ม **"ล้างสะสม Express"** (reset ตอน void RE)
- **★★ บทเรียนสำคัญ (b3007l ถูก revert — commit 5b909a4):** เคยแก้ด้วยการเปลี่ยน **นิยามกลาง** `isReceived` เป็น "IV ต้องอยู่ใน 191" → **"พร้อมออก RE" พุ่ง 2948** เพราะ re_no ที่มาจากแท็บ 1.9.1 **ไม่ถูกเก็บใน 191 cache** → เกือบทุกใบกลายเป็น "ยังไม่รับชำระ" = เสี่ยงส่งออก RE ซ้ำใบที่คีย์แล้ว
  - **กฎที่ได้:** "ไม่มีหลักฐาน ≠ พิสูจน์ว่าไม่มี" — ของแบบนี้ต้องออกแบบให้ **ข้อมูลขาด = คงสถานะเดิม** แล้วเปลี่ยนเฉพาะเมื่อมีหลักฐานขัดแย้ง · **อย่าเปลี่ยนนิยามกลางที่ทั้งระบบใช้ร่วม** ให้ทำเครื่องมือ blast radius แคบแทน
- **เกี่ยวเนื่อง (commit เดียวกันชุดนี้):** อัปแค่ 191 ไฟล์เดียวได้ ไม่ต้องแนบกระเป๋าเงิน (`expressOnly` · b3007e) · เลขเช็คดึงจาก **IV ของออเดอร์เอง** เท่านั้น ไม่ยืมผ่านเลข RE ที่เดา (b3007g) · order_id TikTok 18 หลัก BigSeller/Express corrupt คนละแบบ → เทียบด้วย **13 หลักแรก+3 หลักท้าย** (`ordOidKey`/`incNormKey` · b3007j) · orphan/conflict ที่ order_id ตรง batch = นับ "คีย์แล้ว" (b3007i)
- **APP_BUILD 3007n** · verified: syntax OK · logic test 4 เคส (จับใบผิด/ไม่แตะใบถูก/ไม่แตะใบไม่มีหลักฐาน) · เจ้าของทดสอบล้าง 2-3 ใบ → ขึ้น "พร้อมออก RE" ถูกต้อง
...
- **(3) ★ TikTok parser นับถอน Failed เป็นถอนจริง (commit 443631a):** เคสจริง betra 22/07 Withdrawal 1,240 `Failed` (เงินไม่ออก) → ถูกถอนรวม 23/07 (1,453.56=1,240+213.56) · parser ไม่กรอง status → ใบถอนปลอม + งวดถัดไปขาด = "ยอดยกมา" หลอก · แก้ `bmpParseTiktokWallet` ข้าม Failed/Cancelled (เช็คเฉพาะ status ข้อความ — ตัวเลข = ยอดแตก 2 ช่อง) · **verified ไฟล์จริง มิ.ย.–ก.ค.: QI 13/14 · BETRA 11/12 = 92%** (ขาดเฉพาะใบแรก 03/06 ก่อนไฟล์)
- **(4) ★★ จบวงจร "ลบเอง-อัปเอง-เช็คเวอร์ชันเอง" (commit 50c2f3a):** วันนี้เจ้าของอัปด้วยโค้ดเก่า (browser cache + แบนเนอร์เวอร์ชันถูกปิด) ซ้ำหลายรอบ → การ์ดเพี้ยนทั้งที่โค้ดถูก · แก้ 2 จุด: (ก) **self-check build** — `bmpRunUpload` fetch index.html (no-store) เทียบ `APP_BUILD` ก่อนประมวลผล ไม่ตรง = บล็อก+แจ้ง refresh (ข) **Replace-by-shop** — เลิก dedup/auto-heal ทั้งชุด · อัปไฟล์ = soft-delete ใบถอนของร้านนั้นเฉพาะช่วงวันที่ไฟล์ครอบ แล้ว insert ใหม่ (ร้านอื่น/ช่วงอื่นไม่แตะ) → ไม่ต้องกด "ลบข้อมูลถอน" ก่อนอัปอีก · หมายเหตุ: re-upload = BQ/ติ๊ก "ทำแล้ว" ของใบในช่วงนั้น reset · `APP_BUILD` (b3007c ที่ sb-footer) **ต้องบั๊มทุก push**
- **กู้ TikTok เบญญา:** อัปไฟล์กระเป๋าเงิน TikTok ใหม่ได้เลย (ไม่ต้องลบก่อน — replace-by-shop จัดการ) · ครอบตั้งแต่ มิ.ย. · อยากครบใบแรก 03/06 → export ตั้งแต่ พ.ค. · แผนเต็ม `.claude/plans/dapper-roaming-beaver.md` (ขั้น 4 ยังไม่ทำ: resolve ยอดยกมาเป็นออเดอร์จริงจากทะเบียน)
...
- **แก้:** guard `c5 && /\d/.test(c5)` (เลข IV ต้องมีตัวเลข) + `c14 && /\d/.test(c14)` (เลขเช็คต้องมีตัวเลข) → ข้ามแถว label · หยิบแถว IV/เช็คจริงที่มีตัวเลขแทน
- **verified:** syntax OK · boot 0 error · behavior-preserving (IV/เช็คจริงมีตัวเลขเสมอ) · กระทบเฉพาะแท็บตรวจ 1.9.1 · ไม่ต้อง migration
...
- **known SKU = SKU จากออเดอร์ที่มี `iv_no` แล้ว** (บัญชีคีย์สำเร็จ · iv_no tag จาก 141.RWT verify) → ไม่ต้อง migration/ตารางใหม่ (ดึงจาก `d.rows` ที่โหลดอยู่)
- **`ivrRenderExport`:** `knownSku` (จาก iv_no'd orders) · `newSkuMap` = SKU ใน eligible ที่ไม่อยู่ใน known → **แบนเนอร์เหลืองใน Step 3** (list SKU + จำนวนออเดอร์ที่ใช้) + **ไฮไลต์เซลล์ SKU ในพรีวิว** (พื้นเหลือง + ● + tooltip)
- **ไม่บล็อกส่งออก** (เตือนเฉยๆ · แจ้งบัญชีสร้างรหัสก่อนคีย์) · พอคีย์+อัป 141.RWT สำเร็จ (iv_no ติด) SKU หายจากคำเตือนอัตโนมัติ
- **verified:** syntax OK · boot 0 error · display-only · ไม่ต้อง migration
- **หมายเหตุ:** ถ้ายังไม่เคยอัป 141 เลย known ว่าง → SKU ขึ้นเตือนหมด (จะแม่นขึ้นเมื่อมีประวัติ iv_no)

### 2026-07-27 (7) — ★ CFF วางแผน: จัดแถวติดกัน + ปุ่มลูกตา (ตามแผน/ตามเงินจริง) · ★★ Shopee QI/Betra recon หายทับกัน
- **(A) จัดแถวแพลนติดกัน + แยกโอน:** เดิม "รับ/โอนเข้า" อยู่บน · "จ่าย/โอนออก" อยู่ล่าง (คนละที่ · โอนดูไม่ออกว่าอันเดียวกัน) → รวม 3 แถวติดกันก่อน net: **＋ รับคาดการณ์** (เขียว) · **🔄 โอนระหว่างบัญชี** (ม่วง · ต้นทาง − / ปลายทาง + · รวม 0 "ย้ายที่") · **－ จ่ายคาดการณ์** (ชมพู) · `cffPlannedAgg` แยก inflowBank/outflowBank/xferBank (แยก transfer ออกจาก in/out)
- **(A) ปุ่มลูกตา** (`cffToggleStaffPlan` · `d.staffPlanShow`) ในหัวรายงาน — ซ่อน=เห็นตามเงินจริง (net = opening − due · ไม่รวมแพลน) · แสดง=คาดการณ์ · โชว์เฉพาะเมื่อ interactive + มีรายการวางแผน
- **(B) ★★ bug: Benya Shopee หลายร้าน (QI CARE + BETRA) อัปหลังบ้านทับกันหาย** — ทั้ง 2 ร้าน channel=`shopee` → อัป QI CARE → `ordRunRecon` มอง BETRA เป็น `only_bs` → `ordReconSave` เดิม **ลบ+ทับ order_no ทุกตัว** → BETRA matched กลายเป็น only_bs (หาย)
- **แก้ `ordReconSave`:** แยก `fromFile` (มี be = มาจากไฟล์ที่อัป) vs `onlyBs` (BigSeller มี · ไฟล์นี้ไม่มี) · fromFile ลบ+ใส่ใหม่ (authoritative) · **onlyBs เช็คก่อน — ถ้ามี recon matched อยู่แล้ว (be_raw ไม่ว่าง จากอัปช็อปอื่น) → คงไว้ ไม่ทับ** · แก้ทับกัน + ผลต่างไม่บวมเกิน
- **verified:** syntax OK · boot 0 error · **กู้คืน:** re-upload หลังบ้านแต่ละร้าน 1 ครั้ง → matched คืน + คงอยู่ · ไม่ต้อง migration
...
- **`codLinkParcels` ลำดับใหม่:** (1) **เบอร์โทรตรง** (`codPhoneKey` = 9 หลักท้าย · ลูกค้าหลายออเดอร์→เลือกใบยอด COD ตรง · ยังซ้ำ→multi) · (2) เลขพัสดุ · (3) ชื่อ+ยอด · (4) ยอด · badge "✓ เบอร์ตรง"/"✓ ชื่อ+ยอดตรง"/"~ ยอดตรง"/"⚠ N ตัวเลือก"/"✕ ไม่พบ"
- **verified (ไฟล์จริง):** เบอร์ตรง 8/98 (จำกัดด้วยช่วงวัน — BigSeller ถึง 18/07 · พัสดุ COD ก.ค.ปลายเดือน) · เบอร์+ยอดตรง unique 1 (คุณสิรินัยน์) · syntax OK · **ต้อง push (migration) + re-import BigSeller ครอบวัน COD** → เบอร์โทรเข้าทะเบียน → ลิงก์แม่น
- **เฟส 3 (ยังไม่ทำ):** เลือกคู่เอง + จำ สำหรับเคส multi (ลูกค้าเดียวหลายออเดอร์ยอดซ้ำ)
...
- **verified (node · 4 เคส):** เศษ 5 ออเดอร์ (1281.00) → เจอครบ · 1 ออเดอร์ (116.19) → เจอ · true fraction (191.30 ไม่มี combo) → null → fallback เศษเงินสด · ±2 สตางค์ (150.00 vs 100.01+50) → เจอ
- **O(n·CAP) DP** (CAP = เศษเป็นสตางค์ ~130k · n ออเดอร์วันคร่อม) รันครั้งเดียว/withdrawal เมื่อมีเศษ · Int32Array · syntax OK · **ต้อง re-upload ไฟล์กระเป๋าเงินใหม่**

### 2026-07-26 (5) — ★★ ถอน Marketplace: ยกไป = 0 — หา "ออเดอร์ที่ net = เศษพอดี" (subset-sum) ย้ายไปงวดหน้า
- **เจ้าของ (หงุดหงิด · ต่อจาก (4)):** ยังเหลือ "ยอดยกไป 116.19" — ต้อง **ไม่มี** · ถอนคร่อมวันต้อง "หาเจอว่าออเดอร์ไหน" แล้วเอาออเดอร์นั้นไปงวดถัดไป (ไม่ใช่ยกเศษเงินสด)
- **ต้นเหตุ (4):** ดันแค่ "ออเดอร์ใหม่สุดที่ไม่ทำให้ขาด" → ออเดอร์คร่อมยอดถอนที่ net > เศษ ถูกเก็บไว้ → เหลือเศษเงินสด (116.19) · แต่จริงๆ มีออเดอร์ (หรือชุด) ที่ net = เศษพอดีอยู่ (ไม่ใช่ใหม่สุด)
- **แก้ `bmpGroupWithdrawals` + helper `bmpSubsetForCarry`:** เศษ (carryOut) > 0 → หา **subset ของออเดอร์บวก** (เดี่ยว→คู่→สาม · ไล่ใหม่สุดก่อน) ที่ `net รวม = เศษ` (ปัดสตางค์ ±1) → เจอ = ย้ายทั้งชุดไปงวดหน้า (queue.unshift เรียงเวลา) · **carryOut = 0 · ใบฝากงวดนี้ = ตรง STM เป๊ะ** · หา subset เป๊ะไม่ได้ (ถอนคร่อม "กลางออเดอร์" จริง · เช็คแบ่งครึ่งไม่ได้) → fallback ดันใหม่สุดแบบไม่ให้ขาด (เศษเงินสดเล็กน้อย เลี่ยงไม่ได้)
- **verified (node · 3 เคส):** เคสแนน (order 116.19 กลางคิว) → subset เจอ → ยกไป 0 · ใบฝาก 6,308.70=STM · ย้าย 1 ออเดอร์ 116.19 · คร่อมกลางออเดอร์จริง (191.30 ไม่มี order ตรง) → fallback เศษเงินสด · Shopee เต็ม pool → ไม่กระทบ
- **display/grouping ตอน upload → ต้อง re-upload ไฟล์กระเป๋าเงินใหม่** · syntax OK · O(n³) แต่รันครั้งเดียว/withdrawal เมื่อมีเศษ (n = ออเดอร์บวก ~20-50 · เร็ว)

### 2026-07-26 (4) — ★★ ถอน Marketplace: เช็คยกไป = ย้าย "รายออเดอร์" ไปงวดหน้า (ไม่ยกเป็นเงินสด) → ใบฝากตรง STM
- **เจ้าของแจ้ง:** เช็คที่ยกไปถอนงวดหน้า **ควรอยู่ในใบฝากเช็คงวดถัดไป** — เดิม `bmpGroupWithdrawals` ยก "เศษคร่อมวัน" เป็น **เงินสด** (carryOut) แต่ **เช็คของออเดอร์ยังค้างในงวดนี้** → ใบฝากงวดนี้เกิน STM (งวดตัวอย่าง: เช็ครวม 7,724.05 − ค่าใช้จ่าย 134 = 7,590.05 แต่ STM จริง 6,308.70 · ต่าง = ยกไป 1,281.35 · ฝากตามรายการที่ระบบเลือก = ไม่ตรง)
- **แก้:** หลัง push "ทั้งวัน" เดิม → เพิ่ม loop **push "รายออเดอร์" (เช็ค) ใหม่สุดไปงวดหน้า** จนยอดคงเหลืองวดนี้ = ยอดถอน · ดันเฉพาะออเดอร์บวก (net>0) · ค่าใช้จ่าย (ยกเลิก/โฆษณา net≤0) คงอยู่งวดนี้ · หยุดถ้าดันแล้วงวดนี้จะขาด (ออเดอร์คร่อมยอดถอนจริง = เก็บไว้ ยกเศษเงินสดเล็กน้อยแทน — เช็คแบ่งครึ่งไม่ได้)
- **ผล:** `orders` งวดนี้ = เฉพาะออเดอร์ที่เช็คฝากงวดนี้จริง → sumCheque − ค่าใช้จ่าย = **ตรง STM** · ออเดอร์ยกไป (เช็ค) โผล่ในงวดถัดไป (queue.unshift เรียงเวลาถูก · ก่อน "ทั้งวัน" ที่ดันไปแล้ว) · carryOut เหลือ = เศษเงินสดเล็กน้อย (จากออเดอร์คร่อมจริง) ไม่ใช่ทั้งก้อน
- **verified (node · 3 เคส):** align (order รวมตรงขอบ) → carryOut 0 · ใบฝาก 6,308.70=STM · ย้าย 2 ออเดอร์ 1,281.35 ✓ · คร่อมออเดอร์จริง → เก็บออเดอร์ ยกเศษเงินสด (เลี่ยงไม่ได้) · Shopee ถอนเต็ม pool → ไม่ย้าย ไม่ regression
- **ป้ายการ์ด:** ยอดยกมา "(เศษเงินสดจากงวดก่อน)" · ยอดยกไป "(เศษออเดอร์คร่อมยอดถอน · เช็คไปงวดหน้า)" · **display จาก wd.orders/carry_out ที่ regroup → ต้อง re-upload ไฟล์กระเป๋าเงินใหม่** (grouping ทำตอน upload · ไม่ต้อง migration · schema เดิม)
...
- **แถวยกเลิกโชว์ป้าย ORD_CXL_META** (`🚫 {short}` เช่น "ยกเลิกตรง" · สีเขียว ok / เหลือง warn) + detail = hint ("ข้อมูลตรง · ไม่ต้องทำอะไรต่อ") แทน `cxl_ok` ดิบ
- **verified (หน้าจริง · stub 6 เคส):** ตรงกัน 4 (2 matched + 2 cxl_ok) · ที่ต้องตรวจ 2 (cxl_open + only_be · ตัด cxl_ok 2 ใบออก) · ต้องตรวจอื่นๆ 1 (cxl_open) · ป้าย "🚫 BigSeller ยังไม่ยกเลิก" · **display-only · กระทบตัวเลข recon = 0** (แค่จัดกลุ่ม/ป้าย · matchStats เดิมนับ cxl_ok เป็นแมทอยู่แล้ว) · syntax OK · boot 0 error

### 2026-07-24 (3) — ★★ ถอน Marketplace: 1 งวดถอน = ทุก txn ตั้งแต่ถอนครั้งก่อน (Model A) + ยอดยกมา/ยกไป
- **เจ้าของเลือก Model A** (หลังชี้ว่า 07/01 มีค่าโฆษณา GMV −8,420 แทรก 28/06): ออเดอร์ทุกใบในงวดได้เช็ครับครบ · ค่าโฆษณา+ยกเลิก = ค่าใช้จ่าย · **verified 07/01: เช็ครับ 14,152.77 − ค่าใช้จ่าย 8,986 (โฆษณา 8,420 + ยกเลิก 566) = 5,166.77** (เดิม FIFO เก่าได้ 5,470.08 − 303.31 เพราะหยุดเก็บก่อนถึงค่าโฆษณา)
- **`bmpGroupWithdrawals` รื้อ:** เลิก FIFO "เก็บจนใกล้ยอดถอน" (หยุดที่จุดตัดแรก → ทิ้งครึ่งหลังของงวดหลังค่าโฆษณาก้อนใหญ่) → **เอาทั้ง pool ตั้งแต่ถอนครั้งก่อน** · ถอนบางส่วน (pool>ถอน) ดัน "ทั้งวันใหม่สุด" กลับเป็นยกไป · คร่อมกลางวัน (ตัดไม่ลงวัน) → **carry cash (ยอดยกมา/ยกไป)**: เช็คของออเดอร์ออกงวดที่มันอยู่ · เงินถอนงวดถัดไป
- **สมการการ์ดใหม่:** `ยอดยกมา + รวมเช็ครับ(net+) − ค่าใช้จ่าย − ยอดยกไป = คงเหลือ = STM` (balance เสมอ · โชว์บรรทัดยกมา/ยกไปเฉพาะเมื่อ >0) · `walletExpense = carryIn + sumCheque − withdraw − carryOut`
- **★ verified ครบ 11 งวด (ไฟล์ MM จริง):** ค่าใช้จ่ายไม่ติดลบทุกงวด (เดิม whole-pool ล้วนติดลบ 4 งวด) · ยกไป→ยกมาต่อเนื่องถูก (6/22 ยกไป 2,805.01 → 6/24 ยกมา 2,805.01 · 7/06→7/08 · 7/14→7/15) · 6/03 = ยกมาก่อนไฟล์ (ถอน 16,585 แต่ไฟล์เริ่ม 6/01 · flag shortfall)
- **Migration `zz-bankrec-mp-carry.sql`:** `brec_mp_withdrawals` +`carry_in`/`carry_out`/`carry_shortfall` · insert เพิ่ม 3 ฟิลด์ · `select("*")` โหลดครบ · **ต้อง push ให้ migration รัน + re-upload ไฟล์กระเป๋าเงินใหม่** (regroup)
- **shortfall คำนวณใหม่:** `carryIn + Σเช็ครับ(net+) < ยอดถอน − 1` (ออเดอร์ในไฟล์ไม่ครบ) · กระทบ Shopee = ถอนเต็มกระเป๋า (pool net = ถอน · ยกมา/ยกไป = 0) พฤติกรรมเดิม

### 2026-07-24 (2) — ★ ถอน Marketplace: เช็ครับ = ยอด net เข้ากระเป๋า (ไม่ใช่ gross/IV)
- **เจ้าของแก้ (ตัวเลขจริง TikTok mommam BQ 2607010002 · ถอน 5,166.77):** เช็ครับต้องรวมได้ **5,388.77** − ค่าใช้จ่าย **222** = เข้าแบงค์ 5,166.77 · **verified ไฟล์จริง:** 6/29+6/30 Σ net ออเดอร์บวก = 5,388.77 · Σ ยกเลิก (−88/−38/−29…) = −222 · net รวม = 5,166.77 (= Earnings 1,967.90+3,198.87)
- **แก้:** `sumCheque` เปลี่ยนจาก `Σ express_gross` (ยอด IV) → **`Σ shopee_net` ของออเดอร์บวก** = มูลค่าเช็ครับจริงที่คีย์ (ยอด net ที่เข้ากระเป๋า · ค่าธรรมเนียมแพลตฟอร์มฝังใน net แล้ว ไม่แยกเป็นค่าใช้จ่าย) · `walletExpense = sumCheque − withdraw` = |ยอดลบยกเลิก+โฆษณา| เท่านั้น
- **`bmpExportCsv`:** "มูลค่าเช็ค" + feeTotal ใช้ net เหมือนกัน · **verified (การ์ดจริง):** net บวก 1,039.52 − ยกเลิก 105 = 934.52 = STM ✓ · กระทบ Shopee = โมเดลเดียวกัน (เช็ค = net เข้ากระเป๋า · deposit-matching)

### 2026-07-24 — ★ ถอน Marketplace: แยกออเดอร์ยกเลิก (net≤0) ออกจากเช็ค + สรุปใหม่ + คอลัมน์เลขเช็คจริง
- **เจ้าของแจ้ง 2 เรื่อง (จากภาพ TikTok mommam BQ 2607010002):**
  1. **ยอดติดลบ (−38/−29) = ออเดอร์ยกเลิก** → บัญชีทำ**ใบลดหนี้** ไม่ใช่ดึงมารับชำระ · ยอด − พวกนี้การเงินรวมเป็น**ค่าใช้จ่ายในกระเป๋า**บันทึกทีเดียวตอน BQ · เดิมโผล่เป็นออเดอร์ flag "ยังไม่ออกใบเสร็จ RE" → การ์ดขึ้น "⚠ ไม่ตรง 7" หลอก
  2. **ช่องเลขที่เช็คขึ้นเป็นเลข RE** (RE-RE2606…) → อยากได้เลขเช็คจริงขึ้นต้น TT/SP/LZ
- **`bmpIsCancelledOrder(o)`** = order ที่ `shopee_net ≤ 0` (ยกเลิก/คืนเงิน) · **`bmpEffMismatch` เลิกนับออเดอร์ยกเลิก** (ไม่ใช่ mismatch = ทำใบลดหนี้) → การ์ด "✓ ตรงครบ" + export ไม่ถูกบล็อก
- **การ์ดสรุปใหม่** (`bmpRenderTab`): แยก `orderRows` (net>0 · ผ่านเช็ค) กับ `cancelledRows` (net≤0 · ใบลดหนี้) · กล่อง recon เปลี่ยนเป็น **รวมยอดที่ต้องผ่านเช็ค (Σ gross ออเดอร์จริง) − ค่าใช้จ่ายในกระเป๋าเงิน = คงเหลือเข้าแบงค์ = เงินเข้าแบงค์จริง (STM)** · **`walletExpense = sumCheque − withdraw`** (identity balances เสมอ = ค่าธรรมเนียม+ยอดยกเลิก+โฆษณา รวมกัน = "ยอด − ทั้งหมด") · ยุบ box "+ปรับปรุงในกระเป๋า" (พับเข้าค่าใช้จ่าย) · หัวการ์ด "ออเดอร์ (ผ่านเช็ค) N · ยกเลิก M"
- **ตารางออเดอร์:** คอลัมน์ใหม่ **"เลขที่เช็ค (จริง)"** = `bmpRealCheque(o,channel)` = เลขเช็คจาก Express ถ้ามี ไม่งั้นสร้าง `{TT|SP|LZ}+order_id` (ตรงกับ `bmpParseChequeReport` ที่ reverse prefix) · คอลัมน์ "IV / เลข RE" แยกโชว์เลข RE · **แถวยกเลิกแยกเป็นบล็อก** (พื้นส้ม "↩ ทำใบลดหนี้ · ค่าใช้จ่ายในกระเป๋า") ไม่ปนกับออเดอร์จริง ไม่ขึ้น mismatch
- **`bmpExportCsv`:** `list` = เฉพาะออเดอร์จริง (ตัดยกเลิก) · เลขที่เช็คใช้ `bmpRealCheque` · `feeTotal = Σgross(จริง) − withdraw` (= ค่าใช้จ่ายในกระเป๋า)
- **verified (หน้าจริง · การ์ดสังเคราะห์ 3 จริง + 3 ยกเลิก):** ผ่านเช็ค 1,338.99 − ค่าใช้จ่าย 105.00 = 1,233.99 = STM ✓ · badge "✓ ตรงครบ" (เดิม ⚠ ไม่ตรง 3) · เลขเช็ค TT58466… (ไม่ใช่ RE) · แถวยกเลิกแยกบล็อก · **display-only · ไม่ต้อง migration/re-upload** (จัดกลุ่มตอน render จาก net ที่มีอยู่) · syntax OK · boot 0 error · กระทบ Shopee = ใช้กติกา net≤0 เดียวกัน (ออเดอร์ปกติ net>0 ไม่กระทบ)
- **หมายเหตุ prefix:** ใช้ช่องทาง (TT/SP/LZ) ตรงกับ parser เช็ครับ · ถ้า Express ใช้ prefix รายร้าน (SI/SQ/TQ…) ต่างจากนี้ ค่อยเพิ่ม map — ตอนนี้เลขเช็คแก้ได้ที่แถว mismatch

### 2026-07-23 — ★ จับคู่ยอดถอน Marketplace: รองรับ **TikTok** (แมพ "เงินเข้าแบงค์วันนี้ = ออเดอร์ไหน · เลขที่เช็คอะไร")
- **เจ้าของขอ:** อัปไฟล์ "รายการรับเงิน TikTok" แล้วแมพว่าเงินที่เข้าธนาคารก้อนนี้มาจากออเดอร์ใบไหน เลขที่เช็คอะไร (เหมือนที่ Shopee ทำอยู่ในแท็บ 🛒 ถอน Marketplace)
- **โครงไฟล์ TikTok (2 ชีตที่ใช้):** `รายละเอียดคำสั่งซื้อ` = ออเดอร์ + **วันที่ TikTok ชำระเข้ากระเป๋า** (col "เวลาที่ชำระคำสั่งซื้อ") + เงินเข้าสุทธิ (col "จำนวนเงินที่ชำระทั้งหมด") · `บันทึกการถอน` = **ลำดับเวลาจริงของกระเป๋า** 3 ชนิด `Earnings` (เงินเข้ารายวัน) · `Withdrawal` (ถอนเข้าแบงค์ + เลขบัญชี ****xxxx) · `GMV Pay Deduction` (ค่าโฆษณาหักจากกระเป๋า)
- **★ verified กับไฟล์จริง (MBark 01/06–21/07/69):** Earnings ของวัน = **Σ ออเดอร์ที่ชำระวันนั้นเป๊ะ 51/51 วัน** (หลังแยกค่าโฆษณา) · Σ ออเดอร์ 104,187.35 − ค่าโฆษณา 10,386.46 = **93,800.89 = ยอดในชีต "รายงาน"** · ถอน 11 ครั้ง 94,014.58
- **`bmpParseTiktokWallet(wb)` (ใหม่)** → คืน txn stream **หน้าตาเดียวกับ `bmpParseShopeeBalance`** (`รายรับจากคำสั่งซื้อ` / `รายการปรับปรุง` / `การถอนเงิน`) → **reuse FIFO grouping + gen BQ + จับเลขที่เช็ค (Express รับชำระ/เช็ครับ → fallback order_ledger) + export CSV/Excel + จับกับสเตทเมนต์ + tag กลับทะเบียน ทั้งชุด ไม่ต้องแก้ downstream**
  - ลำดับเวลาอ่านจากชีต "บันทึกการถอน" (ไฟล์เรียง**ใหม่→เก่า** ต้อง reverse) · ออเดอร์ถูกปล่อยเข้าสตรีมตอนเจอแถว `Earnings` ของวันนั้น · datetime สังเคราะห์ `YYYY-MM-DD HH:MM:SS` จาก tick ต่อวัน (sort ซ้ำ downstream แล้วลำดับไม่เปลี่ยน)
  - **gotcha ไฟล์จริง:** บางแถว Earnings **ยอดแตกเป็น 2 ช่อง** (ช่อง "สถานะ" กลายเป็นตัวเลข) → parser บวกรวม · TikTok ไม่มี running balance → `balance_after=0`
- **`BMP_TT_ACCT`** เลขบัญชี 4 ตัวท้ายในชีตถอน → ร้าน (ไฟล์ TikTok **ไม่มีชื่อร้าน**): `9281`→tiktok mommam (MBark SCB) · `1640`→tiktok qi care (Benya SCB) · `0405`→tiktok betra (Benya BBL) · เพิ่ม 3 คีย์นี้ใน `BMP_SHOP_ROUTING` (channel `tiktok`) · บัญชีไม่รู้จัก → `tiktok ****xxxx` (routing ว่าง = ข้าม + แจ้ง)
- **`bmpIsoFromAny` +รูปแบบ `yyyy/mm/dd`** (TikTok ใช้) — ต้องจับ**ก่อน** `bmpThaiDate` (dd/mm/yyyy พ.ศ.) ไม่งั้นปีเพี้ยน
- **★ แก้ FIFO ใน `bmpGroupWithdrawals`:** เพิ่ม "กวาดรายการปรับปรุงที่ค้างหน้าคิว" หลัง loop (ดึงเข้ากลุ่มเฉพาะเมื่อทำให้ยอด**ใกล้ยอดถอนขึ้น**) — เดิม break ทันทีที่ออเดอร์ตัวถัดไปเกิน → **ค่าโฆษณาที่หักวันเดียวกันตกไปงวดหน้า** (เจอชัดใน TikTok) · verified: ข้อมูลจริง Shopee-style ผลเท่าเดิมทุกก้อน (ไม่ regression) · เคสสังเคราะห์ 100+200−30 = ถอน 270 → ยกไป 0 ✓
- **UI:** ช่องอัปเปลี่ยนป้ายเป็น "รายงานกระเป๋าเงิน — Shopee Balance / TikTok รายการรับเงิน" (**auto-detect จากชื่อชีต** ไฟล์ไหนก็วางช่องนี้) · หัวตาราง "รหัสคำสั่งซื้อ Shopee/net Shopee" → "รหัสคำสั่งซื้อ/net กระเป๋า" · เลขเช็คของแถวค่าโฆษณาใช้ prefix `TT` (TikTok) / `SP` (Shopee)
- **★ ธรรมชาติ TikTok ต่างจาก Shopee:** ถอนเป็น**ยอดที่กรอกเอง** ไม่ใช่ถอนทั้งกระเป๋า → ออเดอร์ของวันเดียวถูกแบ่ง 2 ก้อนถอนได้ · FIFO จึงมีเศษ "ยกไป" ±เล็กน้อยเป็นปกติ (ข้อมูลจริง ≤128 บาท · ก้อนแรกของงวดขาดเพราะมียอดยกมาจากเดือนก่อน)
- **ต้องคีย์ IV/RE ก่อนถึงจะได้เลขที่เช็ค** — เลขเช็คมาจาก Express (รับชำระ/เช็ครับ) หรือ `order_ledger.cheque_no/re_no` · ออเดอร์ TikTok ที่ยังไม่คีย์จะขึ้น "ยังไม่คีย์ IV" ในผลตรวจ (เหมือน Shopee) · **กระทบหน้าอื่น = 0** (parser/routing ใหม่ + FIFO เพิ่มเงื่อนไข) · ไม่ต้อง migration
