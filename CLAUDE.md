...
- **(เพิ่มรอบเดียวกัน) ชิปด่วน + ย่อ/ขยายกลุ่มค่าใช้จ่าย:**
  ...
  - **ลดความรก:** เวนเดอร์ที่มี **รายการเดียว → ยุบเป็น 1 บรรทัด** (ชื่อเด่น + subline "เลขเอกสาร — คำอธิบาย" ตัดส่วนซ้ำชื่อเวนเดอร์) ไม่มีหัว/chevron/count · **หลายรายการ → หัวเวนเดอร์เด่น** (พื้น `rgba(brand,.04)` + ตัวหนา brand-dark + chip จำนวน) แล้วค่อยกางรายการย่อย · `subLine(p,ven)` helper ตัด desc ที่ซ้ำชื่อเวนเดอร์ (กัน recurring โชว์ชื่อซ้ำ)
