...
- **Stage ถัดไป (ยังไม่ทำ):** บันทึก recon ลง DB (`order_recon*`) + แท็บประวัติ · order_events log + timeline จริง · IV validation (อัป 723-5 เทียบยอด) · ย้าย order master เป็น `orders` + export AutoKey จากหน้านี้ + header learned-mapping (`import_column_map`) + UI ระบุร้าน (`shop_registry`) · Recon engine + แท็บตรวจรายวัน · IV validation + AR + status board + กราฟรายเดือน + global order search · export AutoKey (reuse `exkExport`)
